...
<?php
	...
	if(isset($_POST['id'])){
		...
		$_SESSION['cpf'] = mysqli_real_escape_string($conn, $_POST['cpf']);
		$_SESSION['descricao'] = mysqli_real_escape_string($conn, $_POST['descricao']);
		$_SESSION['select_advogado'] = mysqli_real_escape_string($conn, $_POST['select_advogado']);
		...
		$select_advogado = $_SESSION['select_advogado'];
...
if($altera == 1){
	$result_usuario = "UPDATE clientes SET nome = '$nome', email = '$email', senha = '$senha', senha_real = '$senha_real', whatsapp = '$whatsapp', cpf = '$cpf', descricao = '$descricao', id_advogado = '$select_advogado', situacoe_id = '$select_situacao', niveis_acesso_id = '$select_nivel_acesso' WHERE id = '$id_ok'";	
	$resultado_usuario = mysqli_query($conn, $result_usuario);
		...
		<!-- Modal -->
			  ...
			  <div class='modal-dialog'>
				<div class='modal-content'>
				  ...
				  <div class='modal-footer'>
					<a href='../../../administrativo.php?link=73&id=".$id_ok."' class='btn btn-sm btn-primary' data-bs-dismiss='modal'>ok</a>
				  </div>
				</div>
			  </div>
			</div>
			...
			unset( $_SESSION['senha_real'] );
			unset( $_SESSION['senha_md5'] );
			unset( $_SESSION['whatsapp'] );
			unset( $_SESSION['cpf'] );
			unset( $_SESSION['descricao'] );
			unset( $_SESSION['select_advogado'] );
	  ...
	  <div class='modal-dialog'>
		<div class='modal-content'>
		  ...
		  <div class='modal-body'>
			...
			<li>CPF/CNPJ: <?php echo "$cpf";?> </li>
			<li>Descrição: <?php echo "$descricao";?> </li>
			</ul>
		  </div>
		  <div class='modal-footer'>
			<a href="adm_proc_edita_clientes.php?id_dados=<?php echo"$id";?>&altera=1" class='btn btn-sm btn-danger' data-bs-dismiss='modal'>sim</a>
			<a href='../../../administrativo.php?link=73&id=<?php echo "$id";?>' class='btn btn-sm btn-success' data-bs-dismiss='modal'>não</a>
		  </div>
		</div>
	  </div>
	</div>
